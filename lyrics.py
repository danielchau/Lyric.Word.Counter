...
for i in range(len(artist_name)):
  if __name__ == "__main__":
    ...
    if song_info:
      ...
      song_lyrics_array.append(song_result)
      
      totallyrics = totallyrics + song_result
    else:
      song_lyrics_array.append("N/A")
      song_dates.append("N/A")

prelim_dict = dict()
final_word_dict = word_count(totallyrics, prelim_dict)
prelim_dict = dict.fromkeys(final_word_dict, 0)

for i in range(len(song_dates)):
  if song_dates[i] != "N/A":
    prelim_dict = dict.fromkeys(final_word_dict, 0)
    song_wordcount_array = word_count(song_lyrics_array[i], prelim_dict)
...
df = pandas.DataFrame.from_dict(final_word_dict, orient='index')
